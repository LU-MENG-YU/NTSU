(function(B){
  ...
  V.Trends.render=(id,context)=>{
    ...
    const summary=U.periodSummary(s.start,s.end,method)+' · '+U.filterText(context.filter?.level,'不分級')+' · '+U.filterText(context.filter?.position,'全部位置')+(context.team?' · 球員等權':'')+(s.unit==='week'?' · 週日～週六，首尾週依選取日期截斷':'');
