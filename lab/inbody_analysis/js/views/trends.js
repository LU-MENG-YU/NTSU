(function(B){
  ...
  V.Trends.render=(id,context)=>{
    ...
    const filterSummary=context.filter?.positionsByLevel?U.levelPositionText(context.filter):U.filterText(context.filter?.level,'不分級')+' · '+U.filterText(context.filter?.position,'全部位置');
    const summary=U.periodSummary(s.start,s.end,method)+' · '+filterSummary+(context.team?' · 球員等權':'')+(s.unit==='week'?' · 週日～週六，首尾週依選取日期截斷':'');
    ...
    root.insertAdjacentHTML('beforeend',U.metricBlock(id+'-plot',title,summary,table,{unit:s.unit,dual:true,team:!!context.team}));
    B.Charts.dualTimeline(id+'-plot',sets,{title,summary,unit:s.unit,team:!!context.team});
