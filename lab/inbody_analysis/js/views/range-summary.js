(function(B){
  ...
  const scope=r=>`${U.filterText(r.filter.level,'不分級')} · ${U.filterText(r.filter.position,'全部位置')}`;
  ...
  const eligible=()=>B.state.engine.players({level:state.level,position:state.position});
  const filteredResult=r=>({...r,rows:r.rows.filter(row=>!state.hidden.has(row.player.name))});
  const result=keys=>{
    const p=periodsFrom(state),r=B.Analysis.rangeSummary(B.state.engine,{filter:{level:state.level,position:state.position},unit:state.unit,...p,keys});
  ...
  const readControls=()=>{
    const next={...state,level:U.multiVal('range-summary-level'),position:U.multiVal('range-summary-position'),unit:U.val('range-summary-unit')};
  ...
  const validate=s=>{
    const p=periodsFrom(s);B.Analysis.rangeSummary(B.state.engine,{filter:{level:s.level,position:s.position},unit:s.unit,...p,keys:defaults});
    return p;
  };
  R.render=id=>{
    ...
    if(source!==data){
      ...
      state={level:[],position:[],unit:'month',startMonth,endMonth,startDay:e.min,endDay:e.max,startWeekMonth:startMonth,startWeek:sWeeks[0]?.index||'1',endWeekMonth:endMonth,endWeek:eWeeks.at(-1)?.index||'1',hidden:new Set(),customOpen:false,customKeys:['weight']};source=data;
    }
    root.className='range-summary';
    root.innerHTML=`<div class="dual-heading"><h2>區間平均與首尾${unitName(state.unit)}差值</h2><p>依球員列出區間平均，以及指定起點${unitName(state.unit)}到終點${unitName(state.unit)}的變化。</p></div><form id="range-summary-form" class="range-summary-controls"><div class="toolbar">${U.multiSelect('range-summary-level','分級',B.levels(data.players).filter(Boolean),state.level,'不分級')}${U.multiSelect('range-summary-position','位置',B.positions(data.players),state.position,'全部位置')}${U.select('range-summary-unit','時間單位',[{value:'month',label:'月 Month'},{value:'week',label:'週（日～六）'},{value:'day',label:'日 Day'}],state.unit)}${periodControls()}<button type="submit" class="primary">更新表格</button></div><p class="conditions">本區獨立篩選，固定採用 Average 平均。週次固定為週日～週六，月初與月底所在週可跨月。</p><p id="range-summary-error" role="alert" hidden></p>${selectorMarkup()}</form><div id="range-summary-results"></div><details id="range-summary-custom" class="panel range-custom" ${state.customOpen?'open':''}><summary>自訂指標表（選用）</summary><p class="conditions">沿用本區分級、位置、顯示球員與起訖期間；可增加其他指標，與預設四項分開顯示。</p><div id="range-summary-custom-controls"></div><div id="range-summary-custom-result"></div></details>`;
    U.wireMulti('range-summary-level');U.wireMulti('range-summary-position');R.wireControls();R.paint();R.paintCustomControls();
