(function(B){
  ...
  V.import=()=>{
    ...
    U.bind('templates','click',()=>{const wb=XLSX.utils.book_new();for(const [name,headers] of [['球員',['背號','姓名','外援','NAME','分級','位置','功能／角色','備註']],['賽季',['賽季名稱','開始','結束']],['身體組成',['name','analysis_date','weight','pbf','ffmi','smm','impedance','left_arm_impedance','right_arm_impedance','left_leg_impedance','right_leg_impedance','trunk_impedance']]])XLSX.utils.book_append_sheet(wb,XLSX.utils.aoa_to_sheet([headers]),name);XLSX.writeFile(wb,'匯入範本.xlsx');});
  ...
  V.importReport=(data,canApply)=>{
    ...
    const impedance=data.registry.filter(m=>m.impedanceFormat==='scalar'||(m.parent&&m.band===1)).map(m=>{
      const scalar=m.impedanceFormat==='scalar',keys=scalar?[m.key]:[1,2,3].map(i=>m.parent+'_'+i);
      const good=data.rows.filter(r=>keys.every(k=>B.valid(r.values[k]))),first=good[0];
      return [B.esc(scalar?m.key:m.parent),scalar?'單一數值':'三波段（1 / 2 / 3）',good.length,data.rows.length-good.length,first?keys.map(k=>B.fmt(first.values[k])).join(' / '):'—'];
    });
    return `<section id="report-summary" class="panel"><div class="panel-title"><h2>${canApply?'驗證結果 · 待套用':'目前資料的匯入結果'}</h2>${data.metadata.demo?'<span class="badge demo">合成示範資料</span>':''}</div>${U.stats([{label:'原始測量列',value:r.total},{label:'納入資料集',value:r.included},{label:'排除紀錄',value:r.excluded.length,foot:`其中 ${r.duplicates} 列完全重複`},{label:'名單球員',value:data.players.length}])}
      ...
      ${B.needsReparse(data)?'<div class="notice warning">這份資料使用舊版解析。請重新加入來源檔案、讀取、驗證並套用，讓 impedance 的單一數值與新版驗證結果生效。</div>':''}
      ...
      ${impedance.length?`<details class="validation-section"><summary>阻抗解析概況（${impedance.length} 個欄位）</summary><p class="small muted">僅計算納入資料集的測量。impedance 為單一數值；各部位阻抗為三波段，依來源順序排列，未假設頻率。欄位缺失不會排除整筆測量。</p>${U.table(['欄位','格式','成功列數','缺失／格式錯誤列數','首筆成功值（Ω）'],impedance)}</details>`:''}
  ...
  V.home=()=>{
    ...
    U.main().innerHTML=U.heading('OVERVIEW','球隊資料總覽',`測量期間 ${e.min} ～ ${e.max}`,`<a class="button" href="#import">重新載入資料</a>`)+
      (B.needsReparse(data)?'<div class="notice warning">目前使用舊版解析的快取。請<a href="#import">重新匯入來源檔</a>，讓新版阻抗解析與驗證結果生效。</div>':'')+
      ...
      <div class="entry-grid"><a class="entry" href="#team"><div class="eyebrow">TEAM</div><h2>團隊分析 →</h2><p>檢視週／月趨勢與跨年度比較</p></a><a class="entry" href="#comparison"><div class="eyebrow">COMPARISON</div><h2>個人分析 →</h2><p>球員比較、四象限與移動軌跡</p></a><a class="entry" href="#report"><div class="eyebrow">PHASE 2</div><h2>年度報告</h2><p>模組介面已保留，後續功能</p></a></div>
