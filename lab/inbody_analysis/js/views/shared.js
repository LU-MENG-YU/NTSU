(function(B){
  ...
  U.wireMulti=id=>{const root=document.getElementById(id);if(!root)return;const inputs=()=>[...root.querySelectorAll(`[data-multi-option="${id}"]`)];const update=()=>{const all=inputs(),checked=all.filter(x=>x.checked),strong=root.querySelector('summary strong');if(strong)strong.textContent=checked.length===all.length?(root.dataset.multiAllLabel||'全部'):checked.length?checked.map(x=>x.closest('label')?.querySelector('span')?.textContent||x.value).join('、'):'未選';};const fire=()=>{update();root.open=false;root.dispatchEvent(new Event('change',{bubbles:false}));};inputs().forEach(input=>input.addEventListener('change',event=>{event.stopPropagation();update();}));root.querySelector(`[data-multi-all="${id}"]`)?.addEventListener('click',event=>{event.stopPropagation();inputs().forEach(x=>x.checked=true);update();});root.querySelector(`[data-multi-none="${id}"]`)?.addEventListener('click',event=>{event.stopPropagation();inputs().forEach(x=>x.checked=false);update();});root.querySelector(`[data-multi-apply="${id}"]`)?.addEventListener('click',event=>{event.stopPropagation();fire();});};
  ...
  U.filter=(filter,year)=>`${U.select('year','年份',B.state.engine.years,year)}${U.multiSelect('level','分級',B.levels(B.state.data.players),filter.level,'不分級')}${U.multiSelect('position','位置',B.positions(B.state.data.players),filter.position,'全部位置')}`;
