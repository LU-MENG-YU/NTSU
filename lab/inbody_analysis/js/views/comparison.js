(function(B){
  ...
  const ensureDefaultMarker=(c,players)=>{
    ...
    if(!group){group={id:'default-'+(++markerSequence),name:'當期球員',color:'#197AA4',open:false,default:true};c.markGroups.unshift(group);}
  ...
  V.paintComparisonCard=(card,commonPack)=>{
    ...
    B.Charts.scatter(id,result,c.x,c.y,{title:title(c),summary,names:c.names,spread:c.spread,previous:c.previous,movement:c.movement,labelOffsets:c.labelOffsets,markStyles:markerStyleMap(c),region:c.region,pair,referenceLabel:monthPair?c.pairBase+' 比較月':weekPair?pack.base.fullLabel+' 起始週':'前一期有效位置',currentLabel:monthPair?c.pairCurrent+' 當期月':weekPair?pack.current.fullLabel+' 終點週':'當期球員',onClick:names=>V.playerMenu(names,id),onLabelMove:(name,pos)=>c.labelOffsets.set(name,pos),legendPosition:c.legendPosition,onLegendMove:pos=>{if(B.valid(pos.x)&&B.valid(pos.y))c.legendPosition=pos;},onLegendEdit:groupId=>{const group=markerGroup(c,groupId);if(!group)return;c.markerOpen=true;group.open=true;V.refreshComparisonCard(id).then(()=>[...document.querySelectorAll('[data-marker-name]')].find(el=>el.dataset.markerName===groupId)?.focus());}});
