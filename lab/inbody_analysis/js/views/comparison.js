(function(B){
  ...
  V.renderComparison=()=>{
    ...
    const {ds,active}=pack,summary=`${active.label} · ${U.filterText(s.filter.level,'不分級')} · ${U.filterText(s.filter.position,'全部位置')} · ${B.methodName(ds.method)}`;
    ...
    U.wireMulti('level');U.wireMulti('position');for(const id of ['level','position','method'])U.bind(id,'change',()=>{s.filter={level:U.multiVal('level'),position:U.multiVal('position')};if(s.scope!=='week')s.method=U.val('method');V.renderComparison();});
  ...
  V.seriesLegendEditor=(c,referenceLabel)=>{const base=ensureDefaultMarker(c,[]),items=[['current','當期球員',base.name,base.color,base.legendVisible!==false],['previous','前一期有效位置',c.seriesLegends.previous.name||referenceLabel,c.seriesLegends.previous.color,c.seriesLegends.previous.visible!==false],['movement','移動軌跡',c.seriesLegends.movement.name||'移動軌跡',c.seriesLegends.movement.color,c.seriesLegends.movement.visible!==false]];return `<details id="${c.id}-series-legend" class="scatter-series-legend" ${c.seriesLegendOpen?'open':''}><summary>圖例名稱、顏色與顯示</summary><p class="small muted">三個基礎圖例可分別改名、改色或隱藏；圖上的圖例文字可各自拖曳位置。</p><div class="series-legend-grid">${items.map(([key,fallback,name,color,visible])=>`<div class="series-legend-row" data-series-legend-row="${key}"><label class="check"><input type="checkbox" data-series-legend-visible="${key}" ${visible?'checked':''}>顯示</label><label>名稱<input type="text" data-series-legend-name="${key}" value="${B.esc(name||fallback)}" maxlength="40"></label><label>顏色<input type="color" data-series-legend-color="${key}" value="${normalizeHex(color)||'#197AA4'}"></label></div>`).join('')}</div></details>`;};
  V.paintComparisonCard=(card,commonPack)=>{
    ...
    const {visible,active}=pack,allPlayers=pack.ds.players;ensureDefaultMarker(c,allPlayers);const ds={...pack.ds,players:allPlayers.filter(p=>!c.hidden.has(p.name))};
    ...
    const result=E.filterScatter(pack.result,c.x,c.y,c.hidden),summary=`${pairSummary} · ${U.filterText(s.filter.level,'不分級')} · ${U.filterText(s.filter.position,'全部位置')} · ${B.methodName(ds.method)}`;
    ...
    root.querySelectorAll('[data-series-legend-visible]').forEach(input=>input.onchange=()=>{const key=input.dataset.seriesLegendVisible;if(key==='current'){const base=ensureDefaultMarker(c,allPlayers);base.legendVisible=input.checked;}else c.seriesLegends[key].visible=input.checked;refresh();});
    root.querySelectorAll('[data-series-legend-name]').forEach(input=>input.onchange=()=>{const key=input.dataset.seriesLegendName,value=B.clean(input.value);if(key==='current'){const base=ensureDefaultMarker(c,allPlayers);base.name=value||'當期球員';}else c.seriesLegends[key].name=value;refresh();});
    ...
    root.querySelectorAll('[data-marker-legend-visible]').forEach(input=>input.onchange=()=>{const g=markerGroup(c,input.dataset.markerLegendVisible);if(g){g.legendVisible=input.checked;refresh();}});
    ...
    root.querySelectorAll('[data-marker-delete]').forEach(button=>button.onclick=()=>{const groupId=button.dataset.markerDelete,base=ensureDefaultMarker(c,allPlayers);c.markGroups=c.markGroups.filter(g=>g.id!==groupId);for(const [name,markId] of [...c.marks])if(markId===groupId)c.marks.set(name,base.id);refresh();});
    root.querySelectorAll('[data-marker-player]').forEach(input=>input.onchange=()=>{const name=input.dataset.markerPlayer,groupId=input.dataset.markerGroupId,group=markerGroup(c,groupId),base=ensureDefaultMarker(c,allPlayers);if(input.checked)c.marks.set(name,groupId);else if(c.marks.get(name)===groupId){if(group?.default)c.marks.delete(name);else c.marks.set(name,base.id);}c.knownPlayers.add(name);refresh();});
