(function(B){
  ...
  const newMarkerGroup=c=>({id:'marker-'+(++markerSequence),name:'標記 '+(c.markGroups.length+1),color:markerPalette[c.markGroups.length%markerPalette.length],open:true});
  ...
  const markerColor=(c,name)=>markerGroup(c,c.marks.get(name))?.color||'#243e53';
  const makeCard=(x,y,delta=false,custom=false)=>({id:'comparison-'+(++sequence),x:{key:x,mode:delta?'delta':'absolute',center:'avg'},y:{key:y,mode:delta?'delta':'absolute',center:'avg'},period:'',names:true,spread:true,previous:false,movement:false,tableOpen:false,basesOpen:false,listOpen:false,markerOpen:false,markGroups:[],custom,comparisonMode:'period',pairBase:'',pairCurrent:'',weekBaseMonth:'',weekBase:'',weekCurrentMonth:'',weekCurrent:'',marks:new Map(),hidden:new Set(),region:{enabled:false,xMin:null,xMax:!delta&&x==='ffmi'?22:null,yMin:null,yMax:!delta&&y==='ffmi'?22:null,color:'#e6af32'},ffmi:22});
  ...
  V.renderComparison=()=>{
    ...
    const {ds,active}=pack,summary=`${active.label} · ${s.filter.level||'不分級'} · ${s.filter.position||'全部位置'} · ${B.methodName(ds.method)}`;
    ...
    for(const id of ['level','position','method'])U.bind(id,'change',()=>{s.filter={level:U.val('level'),position:U.val('position')};if(s.scope!=='week')s.method=U.val('method');V.renderComparison();});
  ...
  V.paintComparisonCard=(card,commonPack)=>{
    ...
    const {visible,active}=pack,allPlayers=pack.ds.players,ds={...pack.ds,players:allPlayers.filter(p=>!c.hidden.has(p.name))};
    ...
    const result=E.filterScatter(pack.result,c.x,c.y,c.hidden),summary=`${pairSummary} · ${s.filter.level||'不分級'} · ${s.filter.position||'全部位置'} · ${B.methodName(ds.method)}`;
    ...
    B.Charts.scatter(id,result,c.x,c.y,{title:title(c),summary,names:c.names,spread:c.spread,previous:c.previous,movement:c.movement,markStyles:markerStyleMap(c),region:c.region,pair,referenceLabel:monthPair?c.pairBase+' 比較月':weekPair?pack.base.fullLabel+' 起始週':'前一期有效位置',currentLabel:monthPair?c.pairCurrent+' 當期月':weekPair?pack.current.fullLabel+' 終點週':'當期球員',onClick:names=>V.playerMenu(names,id)});
    ...
    for(const key of ['names','spread','previous','movement','listOpen'])U.bind(id+'-'+key,'change',()=>{c[key]=document.getElementById(id+'-'+key).checked;refresh();});
    ...
    U.bind(id+'-marker-add','click',()=>{const group=newMarkerGroup(c);c.markGroups.push(group);rememberColor(group.color);c.markerOpen=true;refresh();});
    U.bind(id+'-marker-clear','click',()=>{c.marks.clear();refresh();});
    ...
    root.querySelectorAll('[data-marker-name]').forEach(input=>input.onchange=()=>{const g=markerGroup(c,input.dataset.markerName);if(g){g.name=B.clean(input.value)||'未命名標記';refresh();}});
    ...
    root.querySelectorAll('[data-marker-delete]').forEach(button=>button.onclick=()=>{const groupId=button.dataset.markerDelete;c.markGroups=c.markGroups.filter(g=>g.id!==groupId);for(const [name,id] of [...c.marks])if(id===groupId)c.marks.delete(name);refresh();});
    root.querySelectorAll('[data-marker-player]').forEach(input=>input.onchange=()=>{const name=input.dataset.markerPlayer,groupId=input.dataset.markerGroupId;if(input.checked)c.marks.set(name,groupId);else if(c.marks.get(name)===groupId)c.marks.delete(name);refresh();});
  ...
  V.playerMenu=(input,cardId)=>{
    ...
    d.querySelectorAll('[data-marker-choice]').forEach(btn=>btn.onclick=()=>{card.marks.set(name,btn.dataset.markerChoice);refresh();});d.querySelector('#clear-mark').onclick=()=>{card.marks.delete(name);refresh();};if(!d.open)d.showModal();
  };
})(BB);
