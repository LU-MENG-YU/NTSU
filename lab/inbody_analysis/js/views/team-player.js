(function(B){
  ...
  V.team=()=>{const e=B.state.engine;if(!e)return V.import();teamState={year:e.years.at(-1),filter:{positionsByLevel:U.defaultLevelPositions()},method:'avg',metrics:core(),mode:'recent',unit:'month',customStart:'',customEnd:'',months:[],years:[...e.years]};V.renderTeam();};
  V.rosterCounts=()=>{
  ...
  V.renderTeam=()=>{
    ...
    const groupLabel=`${t.year} ${U.levelPositionText(t.filter)}`;
    ...
    U.main().innerHTML=U.heading('TEAM / TRENDS','團隊分析','每位球員先取得期間代表值，再計算群體平均。')+V.rosterCounts()+`
      ...
      <div id="team-charts">${t.metrics.map((k,i)=>{const m=registry(k),series=shown(k);return U.metricBlock('team-'+i,`${groupLabel}－${m.label}變化`,summary,U.seriesTable(series,m,true),{unit:t.unit,team:true});}).join('')}</div>
    ...
    U.wireLevelPositions();for(const id of ['year',...U.levelPositionIds(),'method','team-mode','team-unit'])U.bind(id,'change',()=>{t.year=U.val('year');t.filter={positionsByLevel:U.readLevelPositions()};if(t.unit!=='week')t.method=U.val('method');t.mode=U.val('team-mode');t.unit=U.val('team-unit');V.renderTeam();});
    ...
    if(t.months.length){const wrapper=document.getElementById('month-results');wrapper.innerHTML=t.metrics.map((k,i)=>{const m=registry(k),series=A.changes(e.team(ds,k).filter(r=>t.months.includes(r.period)));return U.metricBlock('months-'+i,`${U.levelPositionText(t.filter)}－${m.label} ${B.Extras.monthTitle(t.months)}比較`,`${B.Extras.monthTitle(t.months)} · ${B.methodName(t.method)}`,U.seriesTable(series,m,true),{team:true});}).join('');t.metrics.forEach((k,i)=>{const series=A.changes(e.team(ds,k).filter(r=>t.months.includes(r.period)));B.Charts.timeline('months-'+i,series,registry(k),{title:`${U.levelPositionText(t.filter)}－${registry(k).label} ${B.Extras.monthTitle(t.months)}比較`,summary:`${B.Extras.monthTitle(t.months)} · ${B.methodName(t.method)}`,team:true,history:e.team(ds,k).filter(r=>r.period<=t.months.at(-1))});});}
    if(t.years.length){const annual=e.aggregate({filter:t.filter,method:t.method,unit:'year'});const wrapper=document.getElementById('year-results');wrapper.innerHTML=t.metrics.map((k,i)=>{const series=A.changes(e.team(annual,k).filter(r=>t.years.includes(r.period)));return U.metricBlock('annual-'+i,`${U.levelPositionText(t.filter)}－${registry(k).label}年度比較`,`${t.years.join('、')} · ${B.methodName(t.method)}`,U.seriesTable(series,registry(k),true),{unit:'year',team:true});}).join('');t.metrics.forEach((k,i)=>B.Charts.timeline('annual-'+i,A.changes(e.team(annual,k).filter(r=>t.years.includes(r.period))),registry(k),{title:`${U.levelPositionText(t.filter)}－${registry(k).label}年度比較`,summary:`${t.years.join('、')} · ${B.methodName(t.method)}`,team:true,unit:'year',history:e.team(annual,k).filter(r=>r.period<=t.years.at(-1))}));}
