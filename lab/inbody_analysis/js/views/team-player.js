(function(B){
  ...
  V.team=()=>{const e=B.state.engine;if(!e)return V.import();teamState={year:e.years.at(-1),filter:{level:[],position:[]},method:'avg',metrics:core(),mode:'recent',unit:'month',customStart:'',customEnd:'',months:[],years:[...e.years]};V.renderTeam();};
  V.rosterCounts=()=>{
  ...
  V.renderTeam=()=>{
    U.clear();const t=teamState,e=B.state.engine,data=B.state.data,method=t.unit==='week'?'avg':t.method;
    const selectedYearRows=e.byYear.get(t.year)||[],last=selectedYearRows.at(-1)?.date||`${t.year}-12-31`;
    let end=t.mode==='year'?`${t.year}-12-31`:B.date(last.slice(0,7),true,true);
    const dt=new Date(end+'T00:00:00Z');let start=t.mode==='year'?`${t.year}-01-01`:B.iso(Date.UTC(dt.getUTCFullYear(),dt.getUTCMonth()-11,1));
    if(t.mode==='custom'){start=t.customStart||start;end=t.customEnd||end;}
    const historyStart=e.min<start?e.min:start;
    const trendDS=e.aggregate({filter:t.filter,method:t.method,unit:t.unit,start:historyStart,end,periods:A.windowPeriods(historyStart,start,end,t.unit)});
    const shown=key=>e.team(trendDS,key).filter(r=>r.end>=start&&r.start<=end);
    const ds=e.aggregate({filter:t.filter,method:t.method,start:e.min<start?e.min:start,end:end>e.max?end:e.max});
    const allMonths=ds.periods.map(p=>p.key),monthYears=[...new Set(allMonths.map(m=>m.slice(0,4)))];
    const groupLabel=`${t.year} ${U.filterText(t.filter.level,'不分級')} · ${U.filterText(t.filter.position,'全部位置')}`;
    const summary=U.periodSummary(start,end,method)+' · 球員等權'+(t.unit==='week'?' · 週日～週六，首尾週依選取日期截斷':'');
    U.main().innerHTML=U.heading('TEAM / TRENDS','團隊分析','每位球員先取得期間代表值，再計算群體平均。')+V.rosterCounts()+`
      <section id="team-controls" class="panel control-panel"><h2 class="control-title">01 期間趨勢 · 分析設定</h2><div class="toolbar">${U.filter(t.filter,t.year)}${U.method(t.method,t.unit)}${U.select('team-unit','時間單位',[{value:'month',label:'月 Month'},{value:'week',label:'週（日～六）'}],t.unit)}${U.select('team-mode','顯示期間',[{value:'recent',label:'最近 12 個月'},{value:'year',label:'本年度 1–12 月'},{value:'custom',label:'自訂日期區間'}],t.mode)}</div>${t.mode==='custom'?`<div class="toolbar secondary"><label>開始日期<input id="team-start" type="date" value="${start}"></label><label>結束日期<input id="team-end" type="date" value="${end}"></label><button id="team-range-apply" class="primary">套用日期區間</button></div>`:''}<div class="toolbar secondary" style="display:block">${U.metricPicker(t.metrics)}</div></section><p class="conditions">${B.esc(groupLabel+' · '+summary)} · 目前符合名單 ${e.players(t.filter).length} 人</p>
      <div id="team-charts">${t.metrics.map((k,i)=>{const m=registry(k),series=shown(k);return U.metricBlock('team-'+i,`${groupLabel}－${m.label}變化`,summary,U.seriesTable(series,m,true),{unit:t.unit});}).join('')}</div>
      <section id="month-controls" class="panel control-panel"><div class="panel-title"><div><h2>02 自選月份／同期比較 · 分析設定</h2><p>可跨年選取任意多個月份，依時間排序；Δ 比較相鄰選定有效月份。</p></div></div><details><summary>選擇比較月份（${t.months.length}）</summary><div class="actions"><button id="months-clear" class="small">清空選擇</button></div><div class="month-year-groups">${monthYears.map(year=>`<div class="month-year-group"><div class="month-year-heading"><strong>${year} 年</strong><button class="small" data-month-year="${year}" data-year-checked="true">全選 ${year} 年</button><button class="small" data-month-year="${year}" data-year-checked="false">清除此年</button></div><div class="bulk-selection">${allMonths.filter(m=>m.startsWith(year)).map(m=>`<label><input type="checkbox" data-month="${m}" ${t.months.includes(m)?'checked':''}>${Number(m.slice(5))} 月</label>`).join('')}</div></div>`).join('')}</div><button id="months-apply" class="primary small">套用月份比較</button></details></section><div id="month-results"></div>
      <section id="annual-controls" class="panel control-panel"><div class="panel-title"><div><h2>03 年度比較 · 分析設定</h2><p>年度代表值直接使用該球員當年所有測量，之後再計算群體平均。</p></div></div><div class="bulk-selection">${e.years.map(y=>`<label><input type="checkbox" data-compare-year="${y}" ${t.years.includes(y)?'checked':''}>${y}</label>`).join('')}</div><button id="years-apply" class="small">套用年度比較</button></section><div id="year-results"></div><section id="team-dual" class="dual-section"></section>`;
    U.wireMulti('level');U.wireMulti('position');for(const id of ['year','level','position','method','team-mode','team-unit'])U.bind(id,'change',()=>{t.year=U.val('year');t.filter={level:U.multiVal('level'),position:U.multiVal('position')};if(t.unit!=='week')t.method=U.val('method');t.mode=U.val('team-mode');t.unit=U.val('team-unit');V.renderTeam();});
    U.bind('team-range-apply','click',()=>{try{const range=B.range(U.val('team-start'),U.val('team-end'));t.customStart=range.start;t.customEnd=range.end;V.renderTeam();}catch(err){U.toast(err.message);}});
    U.wireMetrics(t.metrics,V.renderTeam);
    t.metrics.forEach((k,i)=>{const m=registry(k),series=shown(k);B.Charts.timeline('team-'+i,series,m,{title:`${groupLabel}－${m.label}變化`,summary,team:true,unit:t.unit,history:e.team(trendDS,k).filter(r=>r.start<=end)});});
    ...
    if(t.months.length){const wrapper=document.getElementById('month-results');wrapper.innerHTML=t.metrics.map((k,i)=>{const m=registry(k),series=A.changes(e.team(ds,k).filter(r=>t.months.includes(r.period)));return U.metricBlock('months-'+i,`${U.filterText(t.filter.level,'不分級')} · ${U.filterText(t.filter.position,'全部位置')}－${m.label} ${B.Extras.monthTitle(t.months)}比較`,`${B.Extras.monthTitle(t.months)} · ${B.methodName(t.method)}`,U.seriesTable(series,m,true));}).join('');t.metrics.forEach((k,i)=>{const series=A.changes(e.team(ds,k).filter(r=>t.months.includes(r.period)));B.Charts.timeline('months-'+i,series,registry(k),{title:`${U.filterText(t.filter.level,'不分級')} · ${U.filterText(t.filter.position,'全部位置')}－${registry(k).label} ${B.Extras.monthTitle(t.months)}比較`,summary:`${B.Extras.monthTitle(t.months)} · ${B.methodName(t.method)}`,team:true,history:e.team(ds,k).filter(r=>r.period<=t.months.at(-1))});});}
    if(t.years.length){const annual=e.aggregate({filter:t.filter,method:t.method,unit:'year'});const wrapper=document.getElementById('year-results');wrapper.innerHTML=t.metrics.map((k,i)=>{const series=A.changes(e.team(annual,k).filter(r=>t.years.includes(r.period)));return U.metricBlock('annual-'+i,`${U.filterText(t.filter.level,'不分級')} · ${U.filterText(t.filter.position,'全部位置')}－${registry(k).label}年度比較`,`${t.years.join('、')} · ${B.methodName(t.method)}`,U.seriesTable(series,registry(k),true));}).join('');t.metrics.forEach((k,i)=>B.Charts.timeline('annual-'+i,A.changes(e.team(annual,k).filter(r=>t.years.includes(r.period))),registry(k),{title:`${U.filterText(t.filter.level,'不分級')} · ${U.filterText(t.filter.position,'全部位置')}－${registry(k).label}年度比較`,summary:`${t.years.join('、')} · ${B.methodName(t.method)}`,team:true,unit:'year',history:e.team(annual,k).filter(r=>r.period<=t.years.at(-1))}));}
    U.main().querySelectorAll('[data-month-year]').forEach(button=>button.onclick=()=>{const year=button.dataset.monthYear;U.main().querySelectorAll('[data-month]').forEach(input=>{if(input.dataset.month.startsWith(year))input.checked=button.dataset.yearChecked==='true';});});
    V.Trends.render('team-dual',{team:true,filter:t.filter,method:t.method,start,end,unit:t.unit});
    U.flushCharts();
  };
  V.player=name=>{
  ...
  V.measurementRows=(records,m)=>{
    const s=playerState,method=s.unit==='week'?'avg':s.method,summary=A.measurementSummary(records,m.key),metrics=[...new Set([...core(),m.key])].map(registry);
    return `<div class="measurement-box"><p class="small">${B.esc(m.label)}：${summary.valid} 筆有效數值 · 最小 ${B.fmt(summary.min)} · 最大 ${B.fmt(summary.max)}。${method==='avg'?'所有使用中且有值的測量納入平均。':'「採用」標記為本期 '+B.methodName(method)+' 的來源；同值並列。'}</p><p class="small muted">停用會排除整筆測量的所有指標，並同步更新團隊、比較與個人分析。紅色列仍可恢復。</p><div class="table-scroll"><table class="raw-measurements"><thead><tr><th>測量日期／時間</th>${metrics.map(x=>'<th>'+B.esc(x.label)+' ('+B.esc(x.unit)+')</th>').join('')}<th>${B.esc(m.label)}取值標記</th><th>來源</th><th>操作</th></tr></thead><tbody>${records.length?records.map(r=>{
      const v=r.values[m.key],isMin=!r.disabled&&B.valid(v)&&v===summary.min,isMax=!r.disabled&&B.valid(v)&&v===summary.max,used=!r.disabled&&B.valid(v)&&(method==='avg'||(method==='min'&&isMin)||(method==='max'&&isMax));
      return `<tr class="${r.disabled?'measurement-disabled':''}" data-measurement-row="${B.esc(r.uid)}"><td>${r.date}<br><span class="small">${B.esc(B.measurementTime(r))}</span></td>${metrics.map(x=>'<td>'+B.fmt(r.values[x.key])+'</td>').join('')}<td>${r.disabled?'<span class="badge disabled-badge">已停用整筆</span>':`${isMin?'<span class="value-tag">最小</span>':''}${isMax?'<span class="value-tag">最大</span>':''}${used?'<span class="value-tag selected">'+(method==='avg'?'納入平均':'採用')+'</span>':''}`}</td><td class="source-cell">${B.esc(r.source.file)} / ${B.esc(r.source.sheet)}<br>第 ${r.source.row} 列</td><td><button class="small ${r.disabled?'':'danger'}" data-toggle-measurement="${B.esc(r.uid)}" data-disabled="${r.disabled?'false':'true'}">${r.disabled?'恢復整筆':'停用整筆'}</button></td></tr>`;
  ...
  V.renderPlayer=()=>{
    U.clear();const s=playerState,p=s.player,e=B.state.engine,method=s.unit==='week'?'avg':s.method;
    const {start,end}=B.range(s.start,s.end);
    const defs=A.windowPeriods(e.min<start?e.min:start,start,end,s.unit);
    const ds=e.aggregate({filter:{name:p.name},method:s.method,unit:s.unit,start:e.min<start?e.min:start,end,periods:defs});
    const summary=U.periodSummary(start,end,method);
    ...
    U.main().innerHTML=U.heading('PLAYER / PROFILE','球員個人檔案','',`<a class="button" href="#home">返回球員名單</a>`)+`
      ...
      <section class="panel control-panel"><h2 class="control-title">期間與指標</h2><div class="toolbar">${U.select('player-mode','期間',[{value:'recent',label:'最新 12 個月'},{value:'year',label:'本年度'},{value:'custom',label:'自訂區間'}],s.mode)}${U.select('player-unit','時間單位',[{value:'day',label:'日 Day'},{value:'week',label:'週（日～六）'},{value:'month',label:'月 Month'},{value:'year',label:'年 Year'}],s.unit)}${U.method(s.method,s.unit)}<label>開始日期<input id="player-start" type="date" value="${start}" ${s.mode==='custom'?'':'disabled'}></label><label>結束日期<input id="player-end" type="date" value="${end}" ${s.mode==='custom'?'':'disabled'}></label>${s.mode==='custom'?'<button id="player-range-apply" class="primary">套用</button>':''}</div><div class="toolbar secondary" style="display:block">${U.metricPicker(s.metrics)}</div></section><p class="conditions">${B.esc(p.name+' · '+summary)} · Δ 比較前一期有效資料；跨缺失期間以虛線連接。</p>
      ${s.metrics.map((k,i)=>{const m=registry(k),series=e.personal(ds,p.name,k).filter(r=>r.end>=start&&r.start<=end);return U.metricBlock('player-'+i,`${p.name}－${m.label}`,summary,V.playerSeriesTable(series,m,i),{unit:s.unit});}).join('')}<section id="player-dual" class="dual-section player-dual-bottom"></section>`;
    ...
    s.metrics.forEach((k,i)=>B.Charts.timeline('player-'+i,e.personal(ds,p.name,k).filter(r=>r.end>=start&&r.start<=end),registry(k),{title:`${p.name}－${registry(k).label}`,summary,unit:s.unit,history:e.personal(s.unit==='week'?ds:e.aggregate({filter:{name:p.name},method:s.method,start:e.min,end,unit:s.unit}),p.name,k)}));V.Trends.render('player-dual',{name:p.name,filter:{name:p.name},method:s.method,start,end,unit:s.unit});V.wireMeasurements();return U.flushCharts();
  };
})(BB);
